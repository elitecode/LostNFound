...
def index(request):
	if request.GET:
		...
		elif request.COOKIES.has_key('email') and request.get_full_path=='/lnf/lost_something/':
...
def foundform(request):
	if request.POST and request.COOKIES.has_key('email'):
		p=request.POST
...
def emailLost(request,oid):
	if request.COOKIES.has_key('email') and request.POST:
...
def editLost(request,oid):
	if request.COOKIES.has_key('email') and request.COOKIES['email']==Lost.objects.get(id=oid).user.email:
		if request.POST:
			p=request.POST
			if not ( p['itemName'] and p['category'] and p['location'] and p['dateLost']):
				return HttpResponse("Error")
...
def deleteFound(request,oid):
	if request.COOKIES.has_key('email') and request.COOKIES['email']==Found.objects.get(id=oid).user.email:
